...
df = pd.read_csv("ASAP_conspecificity_matrix/ASAP_conspecificity_matrix.csv", index_col=0)
...
# Output directory
output_dir = "ASAP_groups_plateau"
...
for threshold in range(START, END + 1):
    ...
    pd.DataFrame(rows).to_csv(
        f"ASAP_groups_plateau/groups_t{threshold}.csv",
        index=False
    )
