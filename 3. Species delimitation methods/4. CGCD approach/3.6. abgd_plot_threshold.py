...
def run(abgd_csv, prefix="vub_threshold_vs_groups"):

    abgd, abgd_col = prepare_df(abgd_csv)
    abgd_mode, abgd_plateau = best_plateau(abgd, abgd_col)
    plot_and_save(
        abgd, abgd_col, abgd_mode, abgd_plateau,
        f"{prefix}_ABGD.pdf",
        line_color="blue")

    print("Saved:")
    print(f"{prefix}_ABGD.pdf")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        abgd_csv = "/ABGD_analysis/threshold_scan/threshold_summary.csv"
